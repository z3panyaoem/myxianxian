...
@implementation M2Tile {
    ...
    /** Record the time(NULL) for deal with long hit. */
    time_t startTime;
}
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    {
        NSLog(@"long hit , score = %d",[GSTATE valueForLevel:self.level]);
        [self removeWithDelay];
        
        
        NSInteger scoresubTemp = 0;
        NSInteger scoresubOnce = 0;
        scoresubTemp = [self.delegateView getnowScore];
        
        if (scoresubTemp >  scoresubOnce)
        {
            scoresubTemp -= scoresubOnce;
        }
        else
        {
            scoresubTemp = 0;
        ...
        [self.delegateView updateScore:scoresubTemp];
    }
}
