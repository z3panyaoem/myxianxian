...
- (void)moveToDirection:(M2Direction)direction
{
    ...
    if (!_keepPlaying && _won) {
        ...
        _keepPlaying = YES;
        [_grid.scene.delegateView endGame:YES];
    }
    ...
    if (![self movesAvailable]) {
        [_grid.scene.delegateView endGame:NO];
    }
}
...
- (void)materializePendingScore
{
    ...
    _pendingScore = 0;
    _isMoveFlg = 0;
    [_grid.scene.delegateView updateScore:_score];
}
